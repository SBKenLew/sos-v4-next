...
function hrvSupp(data, i, days) {
  ...
  let cnt = 0;
  for (let j = i - days + 1; j <= i; j++) { if (+data[j].HRV < base * 0.85) cnt++; }
  return cnt >= days;
}
...
export function computeAll(data, dailyActs) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const dR_d=dScore(d7p.r,d7c.r,50,55),dM_d=dScore(d7p.m,d7c.m,0,45),dS_d=dScore(d7p.s,d7c.s,0,15);
    const dR_w=dScore(w7p.r,d7c.r,50,55),dM_w=dScore(w7p.m,d7c.m,0,45),dS_w=dScore(w7p.s,d7c.s,0,15);
    const dR_m=dScore(m30p.r,m30c.r,50,55),dM_m=dScore(m30p.m,m30c.m,0,45),dS_m=dScore(m30p.s,m30c.s,0,15);
    ...
    const nsiVal = d.NextDayStressorIndex !== '' && d.NextDayStressorIndex != null && !isNaN(+d.NextDayStressorIndex);
    const ppFromNSI = nsiVal ? 1 - (+d.NextDayStressorIndex) : null;
    if (hasRealPro) {
      PR = d._proR != null ? clamp(+d._proR,0,1) : .28;
      PM = d._proM != null ? clamp(+d._proM,0,1) : .28;
      PS = d._proS != null ? clamp(+d._proS,0,1) : .22;
      PP = d._proP != null ? clamp(+d._proP,0,1) : (ppFromNSI ?? .55);
    } else if (nxt) {
      PP = ppFromNSI ?? (+nxt.VitalzScore >= +d.VitalzScore ? .65 : .40);
      PR = +nxt.Recovery   === 1 ? .78 : .28;
      PM = +nxt.MildStress === 1 ? .72 : .28;
      PS = +nxt.Stress     === 1 ? .78 : .22;
    } else {
      PP = ppFromNSI ?? .55;
      PR = +d.Recovery===1?.78:.28; PM = +d.MildStress===1?.72:.28; PS = +d.Stress===1?.78:.22;
    }
    ...
    // Events
    const evts = [];
    const pisVal = calcPIS(data, i);
    const otpVal = calcOTP(d7c, data, i);
    if (pisVal > 0) {
      const ph = i > 0 ? +data[i-1].HRV : 0, ch = +d.HRV, dr = ph > 0 ? (ph-ch)/ph : 0;
      evts.push({ type:'dis', label:'Acute disruption', chips:[dr>.15?`HRV dropped ${(dr*100).toFixed(0)}%`:null,'Physiological instability','PIS active','Score safeguard triggered'].filter(Boolean) });
    }
    if (otpVal > 0 && !evts.length) evts.push({ type:'dis', label:'Overtraining signal', chips:['OTP active',`−${otpVal} pts`,'Recovery load elevated'] });
    if (acts.length > 0 && !evts.length) evts.push({ type:'gain', label:`${acts.length} action${acts.length>1?'s':''} logged`, chips:acts.map(a=>`${a.cat}: ${a.name}`) });
    if (ctx === 'Optimal' && aps > 40 && !evts.length) evts.push({ type:'opt', label:'Optimal adaptive zone', chips:['All targets met','Lifestyle aligned'] });
    ...
    res.push({
      ...
      dR_d, dM_d, dS_d, dR_w, dM_w, dS_w, dR_m, dM_m, dS_m,
      rPct:d7c.r, mPct:d7c.m, sPct:d7c.s,
      ctx, PP, PR, PM, PS, bs, tp:dayTPM, as,
      cls, dss, otp:otpVal, pis:pisVal, evts,
      acts, cluster, rlsRaw,
      _idx:i,
    });
  }
  ...
  for (let i = 0; i < res.length; i++) {
    ...
    res[i].sos = clamp(res[i].aps + res[i].rls + res[i].cls + res[i].dss - (res[i].otp||0) - (res[i].pis||0) - res[i].rdp, 0, 100);
  }
